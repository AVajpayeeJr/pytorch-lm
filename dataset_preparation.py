...
def write_to_file(output_dir, final_vocab, train_sentences, test_sentences, val_sentences):
    """ Writes vocab / train / test / val to a directory with given language code.
    """
    if not os.path.exists(output_dir + '/'):
        os.makedirs(output_dir)

    with codecs.open(output_dir + '/vocab.txt', 'w', encoding='utf-8') as vocab_file:
        for i in final_vocab:
            vocab_file.write(i + '\n')

    with codecs.open(output_dir + '/train.txt', 'w', encoding='utf-8') as train_file:
        for i in train_sentences:
            train_file.write(' '.join(i) + '\n')

    with codecs.open(output_dir + '/test.txt', 'w', encoding='utf-8') as test_file:
        for i in test_sentences:
            test_file.write(' '.join(i) + '\n')

    with codecs.open(output_dir + '/valid.txt', 'w', encoding='utf-8') as valid_file:
        for i in val_sentences:
            valid_file.write(' '.join(i) + '\n')


def write_brown_clusters(output_dir, num_clusters, clustering_home_path):
    ...
    try:
        ...
        with codecs.open(str(output_dir) + '/' + str(num_clusters) + '_clusters.txt', 'w') as out_file:
        ...
        os.remove(str(output_dir) + '/train.txt.int')
        os.remove(str(output_dir) + '/train.txt.strdb')
        shutil.move(output_dir + '/paths', str(output_dir) + '/' + str(num_clusters) + '_cluster_paths')
...
def main():
    ...
    if args.cluster:
        write_brown_clusters(output_dir=args.output_dir, num_clusters=args.num_clusters,
